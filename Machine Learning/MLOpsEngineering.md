...
* SageMaker downloads data from the S3 bucket to the container's file system. When the training job is complete, it uploads the trained model artifact from local folder to the specified location in S3 and stops the training container.
