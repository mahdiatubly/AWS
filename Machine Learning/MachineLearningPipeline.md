...
4. Asynchronous inference queues incoming requests for asynchronous processing. This option is ideal for requests with large payload sizes (up to 1 GB), long processing times (up to 1 hour), and near-real-time latency requirements. You can use Asynchronous Inference to save on costs by autoscaling the instance count to zero when you have no requests to process. To create an asynchronous inference endpoint, specify the AsyncInferenceConfig object while creating your endpoint configuration. To invoke the endpoint, you place the request payload in Amazon S3 and provide an S3 URL to this payload as part of the InvokeEndpointAsync request. On invocation, SageMaker uses Amazon Simple Queue Service (amazon SQS) to queue the request for processing and returns an identifier and output location as a response. After processing, SageMaker places the result in the Amazon S3 location. You can optionally choose to receive success or error notifications with Amazon Simple Notification Service (Amazon SNS).
